-- =====================================================
-- Migration 025: process_checkout v3 — promo codes, tax, billing/shipping
--                snapshots, manual-payment status
--
-- Supersedes scripts/020. Keeps the same (p_items, p_customer) signature and
-- grant. New p_customer keys (all optional):
--   payment_method   'cod' | 'online'      (online = manual bank transfer)
--   promo_code       text
--   shipping_method  text
--   state, postal_code, country            (shipping snapshot)
--   billing_*        full billing snapshot (falls back to shipping)
--
-- Pricing order (mirrors lib/pricing.orderTotals):
--   subtotal (per-product discounted) → promo discount → online 5% (on the
--   post-promo subtotal) → + shipping (free at 3+ items, else Rs 230)
--   = grand total. (No tax.)
--
-- Snapshots are written onto the order so historical orders never change when a
-- profile is edited later. Promo usage is incremented atomically here. Online
-- orders are created 'pending_payment' + a payment_verifications row; COD orders
...
BEGIN
    ...
    v_status := CASE WHEN v_payment = 'online' THEN 'pending_payment' ELSE 'confirmed' END;
