-- =====================================================
-- Migration 030: free shipping needs 3+ items AND subtotal ≥ Rs 3500
--
-- process_checkout v5 — identical to v4 (028) except free shipping now requires
-- BOTH: at least 3 items AND a subtotal of at least 3500. Mirrors lib/pricing.js
-- (FREE_SHIPPING_MIN_ITEMS + FREE_SHIPPING_MIN_SUBTOTAL). Additive + idempotent.
-- =====================================================
...
BEGIN
    ...
    -- ---- Totals (free shipping when 3+ items AND subtotal >= 3500) ----
    ...
    END IF;
    v_shipping := CASE WHEN v_item_count >= 3 AND v_subtotal >= 3500 THEN 0 ELSE 230 END;
