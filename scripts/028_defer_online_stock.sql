-- =====================================================
-- Migration 028: defer stock commitment for bank-transfer orders
--
-- Bank-transfer (online) orders must NOT hold inventory until the money is
-- confirmed. This migration:
--   * adds orders.stock_committed (has this order's stock been decremented?)
--   * process_checkout v4: COD decrements stock at checkout (final immediately);
--     online orders are created WITHOUT touching stock (stock_committed = false),
--     but stock is still *verified* so we never accept an impossible order.
--   * confirm_order_payment(): the single, atomic commit point for online orders
--     — locks each variant, RE-CHECKS stock (it may have sold out since
--     checkout), decrements it, flips the order to 'confirmed', and marks the
--     payment approved. Idempotent via stock_committed so it can never
--     double-decrement.
--
-- Supersedes the stock logic in scripts/025. Additive + idempotent.
-- =====================================================
...
-- Existing rows already had their stock decremented under the old logic — mark
-- them committed so re-confirming an old order never double-decrements.
UPDATE public.orders SET stock_committed = true WHERE stock_committed = false;
...
BEGIN
    ...
    -- Stock is verified for BOTH payment methods (never accept an impossible
    -- order), but only COD actually reserves it here.
    FOR v_item IN
    ...
    -- ---- Promo code (validated + incremented here, never trusted from client) ----
    ...
    END IF;
    v_shipping := CASE WHEN v_item_count >= 3 THEN 0 ELSE 230 END;
    ...
    VALUES (
        ...
        v_is_cod   -- COD reserves stock now; online commits it at approval
    )
...
-- ---------- confirm_order_payment(): commit stock when money is confirmed ----------
-- The single atomic point where a bank-transfer order's stock is decremented.
-- Locks each variant, RE-CHECKS stock (it may have sold out since checkout),
-- decrements, confirms the order, and marks the payment approved. Idempotent:
-- if stock is already committed it only ensures the approval is recorded.
-- Backend-only (service_role); the API route enforces admin.
...
BEGIN
    ...
    -- Already committed → just make sure the payment is recorded as approved.
    ...
    -- Lock every variant and re-verify stock before committing anything.
    FOR v_item IN
        SELECT variant_id, quantity FROM order_items WHERE order_id = p_order_id
    LOOP
        IF v_item.variant_id IS NULL THEN CONTINUE; END IF;  -- custom items have no variant
        SELECT pv.stock_quantity, p.name, pv.color, pv.size
          INTO v_stock, v_name, v_color, v_size
          FROM product_variants pv JOIN products p ON p.id = pv.product_id
         WHERE pv.id = v_item.variant_id
         FOR UPDATE OF pv;
        IF NOT FOUND THEN
        ...
        IF v_stock < v_item.quantity THEN
            RETURN jsonb_build_object(
                ...
                'name', v_name, 'color', v_color, 'size', v_size,
                'available', v_stock, 'requested', v_item.quantity
            );
        END IF;
    END LOOP;

    -- All good — decrement now that payment is confirmed.
    FOR v_item IN
        SELECT variant_id, quantity FROM order_items WHERE order_id = p_order_id
    LOOP
    ...
    UPDATE orders SET status = 'confirmed', stock_committed = true WHERE id = p_order_id;
...
REVOKE ALL ON FUNCTION public.confirm_order_payment(uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.confirm_order_payment(uuid, uuid, text) TO service_role;
