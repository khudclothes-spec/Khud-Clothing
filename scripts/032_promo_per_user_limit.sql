-- =====================================================
-- Migration 032: per-USER promo usage limits
--
-- Before this, promo_codes.max_uses was a GLOBAL cap: `current_uses` counted
-- every redemption by anyone, so a max_uses of 3 meant only 3 total orders
-- across all customers could ever use the code.
--
-- New behaviour: max_uses is the limit PER user. A max_uses of 3 lets EACH
-- signed-in customer use the code up to 3 times. Enforcement is backed by a new
-- promo_code_redemptions ledger (one row per successful redemption). This
-- applies to every promo type (percentage / fixed / student) — they all share
-- the same validate_promo_code + process_checkout path.
--
-- current_uses is kept as a running GLOBAL total for admin reporting only; it no
-- longer gates redemption.
--
-- Recreates: validate_promo_code (per-user check) and process_checkout v7
-- (per-user check + writes a redemption row). Additive + idempotent.
-- =====================================================
...
-- The per-user count query hits this on every validate/checkout.
CREATE INDEX IF NOT EXISTS idx_promo_redemptions_code_profile
  ON public.promo_code_redemptions(promo_code_id, profile_id);

ALTER TABLE public.promo_code_redemptions ENABLE ROW LEVEL SECURITY;

-- Admins can see the full ledger; customers can see only their own rows. Writes
-- happen exclusively through the SECURITY DEFINER checkout RPC, so there is no
-- INSERT policy for regular users (they can never fabricate a redemption).
DROP POLICY IF EXISTS "Admins manage promo redemptions" ON public.promo_code_redemptions;
CREATE POLICY "Admins manage promo redemptions"
ON public.promo_code_redemptions FOR ALL
USING (public.get_my_role() = 'admin')
WITH CHECK (public.get_my_role() = 'admin');

DROP POLICY IF EXISTS "Users view own promo redemptions" ON public.promo_code_redemptions;
...
-- ---------- validation (definer) — per-user limit ----------
...
-- ---------- process_checkout v7 — per-user promo limit + redemption ledger ----------
-- Identical to v6 (031) except the promo block now caps by per-user redemptions
-- instead of the global current_uses, and records a redemption row after the
-- order is created.
...
DECLARE
    ...
    v_promo_used   integer := 0;          -- this user's prior redemptions of the code
...
BEGIN
    ...
    -- ---- Promo code (per-user cap, validated + recorded here) ----
    ...
    IF v_promo_in <> '' THEN
        ...
        IF FOUND THEN
            -- How many times THIS user has already redeemed it (row-locked above,
            -- so concurrent checkouts by the same user serialise here).
            SELECT COUNT(*) INTO v_promo_used
              FROM promo_code_redemptions
             WHERE promo_code_id = v_promo.id AND profile_id = v_uid;

            IF v_promo.is_enabled
               AND (v_promo.expires_at IS NULL OR v_promo.expires_at >= NOW())
               AND (v_promo.max_uses IS NULL OR v_promo_used < v_promo.max_uses)
            ...
            THEN
                ...
                IF v_student THEN
                    ...
                    -- Global running total (reporting only; no longer gates use).
    ...
    -- ---- Record the per-user redemption (only when a promo actually applied) ----
    IF v_promo_code IS NOT NULL THEN
        INSERT INTO promo_code_redemptions (promo_code_id, profile_id, order_id)
        VALUES (v_promo.id, v_uid, v_order_id);
    END IF;
