...
- **Domain**: production domain is **`https://charmeem.com`** — now the hard fallback in [lib/seo.js](lib/seo.js) `SITE_URL` and [lib/email/config.js](lib/email/config.js) `siteUrl()` (env `NEXT_PUBLIC_SITE_URL` still overrides; `.env.local` was updated from `www.khudclothes.com` → `charmeem.com`; **Vercel env must be updated too, plus a 301 from the old domain**).
