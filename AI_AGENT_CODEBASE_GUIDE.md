...
- Shipping: `SHIPPING_FLAT` Rs 230, **free only when BOTH `FREE_SHIPPING_MIN_ITEMS` (3)+ items AND subtotal ≥ `FREE_SHIPPING_MIN_SUBTOTAL` (Rs 3500)**. `isFreeShipping(itemCount, subtotal)` / `shippingFor(itemCount, subtotal)` / `freeShippingRemaining(itemCount, subtotal)` drive it; the announcement bar + the bag hint ("add N items & Rs X…") read from these. The `process_checkout` RPC mirrors it (`CASE WHEN v_item_count >= 3 AND v_subtotal >= 3500 THEN 0 ELSE 230`, migration [030](scripts/030_free_shipping_subtotal.sql)).
...
- **Favicon = the Khud wordmark** (the old `app/icon.svg` K monogram was deleted). Declared as manual theme-aware `<link>` tags in [app/layout.jsx](app/layout.jsx) `<head>`: `logo-white-writing.png` by default / on dark browser themes, auto-switching to `logo-black-writing.png` on light themes so it's always legible. Manifest icon switched to the black logo (its splash bg is light bone). Rule of thumb kept everywhere: **white logo on dark surfaces, black on light** (email header, footer, transition overlay, admin sidebar already follow this).
- **Link previews**: [app/opengraph-image.jsx](app/opengraph-image.jsx) now embeds the real **black-writing logo** (read from `public/images` and base64-inlined) centred on bone; `SITE_DESCRIPTION` ([lib/seo.js](lib/seo.js)) rewritten for a compelling preview snippet.
