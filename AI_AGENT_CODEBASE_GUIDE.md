# Char Meem Clothing Codebase Guide
...
Char Meem Clothing (formerly Khud) is a Next.js 16 App Router storefront for a premium clothing brand.
...
- **Favicon = the full white-bg artwork, downscaled** (superseded twice — see the July 2026 asset pass section at the end for the current state). All icons are Next.js file conventions generated with `sharp` from [public/images/charmeem-logo-white-bg.jpg](public/images/charmeem-logo-white-bg.jpg) as plain downscales of the full artwork (user-mandated — no cropping or derived marks): [app/favicon.ico](app/favicon.ico) (16/32/48 PNG-in-ICO), [app/icon.png](app/icon.png) (192×192 — a multiple of 48 as Google Search prefers), [app/apple-icon.png](app/apple-icon.png) (180×180), plus [public/images/icon-192.png](public/images/icon-192.png) / [icon-512.png](public/images/icon-512.png) / [icon-512-maskable.png](public/images/icon-512-maskable.png) for the manifest (`any` + padded `maskable`). White background chosen deliberately so the mark reads in Google Search results and dark browser tabs alike. **Site UI logo system**: black transparent logo on light surfaces, gold transparent on dark (SiteShell, auth pages, admin sidebar, emails).
- **Link previews**: static [app/opengraph-image.jpg](app/opengraph-image.jpg) + [app/twitter-image.jpg](app/twitter-image.jpg) (1200×630, gold wordmark centred on black `#070707`, built from `charmeem-logo-gold-black-bg.jpg`; alt via the sibling `.alt.txt` files) replaced the old dynamic `opengraph-image.jsx` and later the white-bg PNGs. `OG_IMAGE` in [lib/seo.js](lib/seo.js) is the shared descriptor: **any page that defines its own `openGraph` must include `images: [OG_IMAGE]` and `type: "website"`** — a page-level `openGraph` replaces the inherited object, silently dropping og:image/og:type otherwise (about, shop, shop/[slug], customize, size-guide do this; product pages use the product photo with `OG_IMAGE` as fallback).
...
- **Route files:** [app/sitemap.js](app/sitemap.js) (static routes + all active products/categories from Supabase, ISR 1h), [app/robots.js](app/robots.js) (allow all, disallow admin/account/checkout/api/auth, points at sitemap), [app/manifest.js](app/manifest.js) (`/manifest.webmanifest`, monogram-on-white icon set incl. maskable), [app/opengraph-image.jpg](app/opengraph-image.jpg) + [app/twitter-image.jpg](app/twitter-image.jpg) (static 1200×630 gold-on-black brand images, auto-applied site-wide; product pages override with the product photo; see the Polish-pass favicon/OG bullets above).
...
### Brand rename: Khud → Char Meem Clothing (July 2026)
- All **user-facing** branding now reads **Char Meem Clothing** ("Char Meem" alone in casual prose, e.g. "Why Char Meem", "Char Meem tees"): metadata/titles ([app/layout.jsx](app/layout.jsx), every page), JSON-LD Organization/WebSite/Product via `SITE_NAME` in [lib/seo.js](lib/seo.js), manifest (`short_name: "Char Meem"`), OG/twitter alt text, logo `alt` attributes, footer copyright, checkout terms line, all Resend email templates ([lib/email/](lib/email/), [emails/](emails/)) and the Supabase auth templates in [docs/supabase-email-templates/](docs/supabase-email-templates/) (must be re-pasted into the Supabase dashboard, including the new subjects).
- **Brand story** (About page + `aboutBlocks` in [lib/data.js](lib/data.js)): four founders, every first name starts with M; "Char Meem" is Urdu for "four Ms". Tagline "Wear your imprint." was kept.
- **Title template fix folded in**: pages no longer hard-code the "— Brand" suffix in `title` (the root template `%s — Char Meem Clothing` appends it; previously suffixes doubled). The home page uses `title: { absolute: ... }`. Page-level `openGraph.title` still needs the full suffix spelled out (templates don't apply to OG).
- **Intentionally NOT renamed** (do not "fix" these): `khud:cart` localStorage key + `khud:add-cart`/`khud:open-cart` window events in [components/CartContext.jsx](components/CartContext.jsx) (renaming would orphan existing shoppers' carts), `[khud]` console.log tags in [app/login/LoginForm.jsx](app/login/LoginForm.jsx), `khud-frontend` package name, the `khudclothes@gmail.com` support inbox + `orders@4MeemClothing.com` Resend sender (live email infrastructure — swap only when charmeem.com mailboxes/domain are verified), and the `KHD-` order-number prefix.

### New brand assets + full SEO/GEO pass (July 19, 2026) — uses the `seo` skill
- **The four supplied artworks** (logo art reads "4 Meem Clothing" — char is Urdu for 4; brand *name* everywhere in text stays **Char Meem Clothing**) were renamed to clean SEO filenames in [public/images/](public/images/): `charmeem-logo-white-bg.jpg` (white bg, black writing — favicon/Google source), `charmeem-logo-gold-black-bg.jpg` (black bg, gold writing — OG/link-preview source), `charmeem-logo-black.png` (transparent, black — site logo on light surfaces AND the Schema.org Organization `logo`, which Google requires to render well on white), `charmeem-logo-gold.png` (transparent, gold — logo on dark surfaces).
- **Logo placement rule (user-mandated, do not invert)**: black (`charmeem-logo-black.png`) on light/bone surfaces — header, mobile menu, auth cards, about hero, home watermark; gold (`charmeem-logo-gold.png`) on dark/ink surfaces — page-transition overlay, footer, admin sidebar, email headers ([lib/email/config.js](lib/email/config.js) `logoUrl`) and both [docs/supabase-email-templates/](docs/supabase-email-templates/) (**must be re-pasted into the Supabase dashboard**). `.admin-logo`'s old `brightness(0) invert(1)` filter was removed from [app/globals.css](app/globals.css) so the gold isn't forced white. A generated `charmeem-logo-white.png` variant existed briefly and was **deleted at the user's request — never derive new logo artwork; use only the supplied images**.
- **All icon files are plain downscales of `charmeem-logo-white-bg.jpg`** (full artwork, no monogram crop — user-mandated): [app/favicon.ico](app/favicon.ico) (16/32/48), [app/icon.png](app/icon.png) (192), [app/apple-icon.png](app/apple-icon.png) (180), [public/images/icon-192.png](public/images/icon-192.png), [icon-512.png](public/images/icon-512.png); the maskable [icon-512-maskable.png](public/images/icon-512-maskable.png) is the same image at 80% on white so Android's circular crop doesn't clip the wordmark. Regenerate via `sharp` (in `node_modules`) rather than editing binaries.
- **Domain (IMPORTANT)**: the live production domain is still **`https://www.khudclothes.com`** — that is the hard fallback in [lib/seo.js](lib/seo.js) `SITE_URL` and [lib/email/config.js](lib/email/config.js) `siteUrl()`, and the value in `.env.local`. The brand *name* is "Char Meem Clothing" and the owner intends to move to **charmeem.com eventually, but has NOT registered it yet** — it does not resolve. Do **not** set any runtime URL (env, Supabase Auth Site URL, redirect allow-list) to charmeem.com until the domain is actually live, or password-reset links and canonicals point at a dead host (symptom: clicking the reset email shows the browser "can't reach this site" page). When charmeem.com goes live: buy it → point DNS at Vercel → set `NEXT_PUBLIC_SITE_URL` in Vercel → update Supabase Auth Site URL + redirect allow-list → 301 the old domain.
- **GEO additions**: [public/llms.txt](public/llms.txt) (plain-language brand summary for AI crawlers); enriched `organizationSchema` in [lib/seo.js](lib/seo.js) — `@id`, `alternateName` (bridges "Char Meem" ↔ "4 Meem"), `email`/`contactPoint` (mirrors `SUPPORT_EMAIL`), PK postal address, `image`; `websiteSchema` gains `@id` + `publisher` linking to the Organization node. `sameAs` is still `[]` — fill it (and the footer placeholder social links in [components/SiteShell.jsx](components/SiteShell.jsx)) when real profiles exist.
