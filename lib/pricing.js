...
export const FREE_SHIPPING_OVER = 3499;         // free shipping when the subtotal is strictly over this
...
// Free shipping on orders strictly over FREE_SHIPPING_OVER (item count no
// longer matters). Otherwise the flat fee applies.
export function isFreeShipping(subtotal) {
  return Number(subtotal) > FREE_SHIPPING_OVER;
}

export function shippingFor(subtotal) {
  return isFreeShipping(subtotal) ? 0 : SHIPPING_FLAT;
}

// Rs still missing to unlock free shipping (0 once unlocked).
export function freeShippingRemaining(subtotal) {
  return {
    amount: Math.max(0, FREE_SHIPPING_OVER + 1 - Number(subtotal || 0))
  };
}
...
// free shipping when `subtotal` > 3499 (pre-promo, like the RPC).
export function orderTotals({ subtotal, paymentMethod = "cod", promoDiscount = 0 }) {
