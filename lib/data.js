...
// --- Mockup image system ------------------------------------------------------
// Static mockup PNGs live at:
//   /public/mockups/<mockupKey>/<colorKey>/<viewFile>.png
// Changing garment / colour / view recomputes this path; if the file is missing
// the studio falls back to the SVG silhouette (GarmentMockup).
...
export function studioMockupSrc(mockupKey, colorKey, view) {
  if (!mockupKey || !colorKey) return null;
  const file = STUDIO_VIEW_FILES[view];
  if (!file) return null;
  return `/mockups/${mockupKey}/${colorKey}/${file}.png`;
}
