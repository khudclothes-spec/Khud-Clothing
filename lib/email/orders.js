// Order email orchestration. Normalises a DB order row into a template model
// and sends the customer confirmation, the four owner notifications, and status
// updates. Server-side only.

import { sendEmail, sendEmails } from "./client";
import { brand, contact, ownerEmails } from "./config";
import { paymentLabel } from "@/lib/pricing";
import { renderCustomerOrderConfirmation } from "@/emails/customerOrderConfirmation";
import { renderOwnerOrderNotification } from "@/emails/ownerOrderNotification";
import { renderOrderStatusUpdate, shouldEmailStatus, STATUS_COPY } from "@/emails/orderStatusUpdate";
import { renderPaymentStatusUpdate } from "@/emails/paymentStatusUpdate";
import { renderPaymentSubmittedCustomer, renderPaymentSubmittedOwner } from "@/emails/paymentSubmitted";

function fmtDate(iso) {
  try {
    return new Date(iso).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" });
  } catch {
    return "";
  }
}

function fmtDateTime(iso) {
  try {
    return new Date(iso).toLocaleString("en-GB", {
      day: "2-digit",
      month: "short",
      year: "numeric",
      hour: "2-digit",
      minute: "2-digit"
    });
  } catch {
    return "";
  }
}

function money(n) {
  return `Rs ${Number(n ?? 0).toLocaleString("en-US")}`;
}

// DB order (+ joined order_items -> products) -> template model.
export function buildOrderModel(order) {
  const items = (order.order_items ?? []).map((it) => ({
    name: it.products?.name ?? "Item",
    color: it.color ?? "",
    size: it.size ?? "",
    quantity: Number(it.quantity) || 0,
    unitPrice: Number(it.unit_price) || 0,
    lineTotal: (Number(it.unit_price) || 0) * (Number(it.quantity) || 0)
  }));

  const name = order.customer_name || "";
  return {
    id: order.id,
    orderNumber: order.order_number,
    orderDate: fmtDate(order.created_at),
    orderDateTime: fmtDateTime(order.created_at),
    customerName: name,
    customerFirstName: name.trim().split(/\s+/)[0] || "",
    customerEmail: order.customer_email || "",
    customerPhone: order.customer_phone || "",
    shippingAddress: order.shipping_address || "",
    city: order.city || "",
    notes: order.notes || "",
    subtotal: Number(order.subtotal) || 0,
    shippingCost: Number(order.shipping_cost) || 0,
    discount: Number(order.discount_amount) || 0,
    promoDiscount: Number(order.promo_discount) || 0,
    promoCode: order.promo_code || "",
    tax: Number(order.tax) || 0,
    paymentMethod: order.payment_method || "cod",
    paymentMethodLabel: paymentLabel(order.payment_method),
    total: Number(order.total_amount) || 0,
    totalFormatted: money(order.total_amount),
    itemCount: items.reduce((t, i) => t + i.quantity, 0),
    items
  };
}

// Customer order confirmation. COD: sent at checkout. Bank transfer: sent later,
// once an admin confirms the order. One recipient (the customer).
export async function sendCustomerOrderConfirmation(dbOrder) {
  const model = buildOrderModel(dbOrder);
  if (!model.customerEmail) return { skipped: true };
  return sendEmail({
    to: model.customerEmail,
    subject: `Your Char Meem Clothing order is confirmed — ${model.orderNumber}`,
    html: renderCustomerOrderConfirmation(model),
    replyTo: contact.supportEmail
  });
}

// "New order placed" notification to each of the four owners — sent once, when
// the order is placed, for BOTH COD and bank transfer (so owners always know an
// order came in; the owner email shows the payment method).
export async function sendOwnerNewOrderEmails(dbOrder) {
  const model = buildOrderModel(dbOrder);
  const ownerHtml = renderOwnerOrderNotification(model);
  const messages = ownerEmails().map((owner) => ({
    to: owner,
    subject: `New order ${model.orderNumber} — ${model.totalFormatted} (${model.paymentMethodLabel})`,
    html: ownerHtml,
    replyTo: model.customerEmail || undefined
  }));
  if (!messages.length) return { sent: 0, failed: 0 };
  const results = await sendEmails(messages);
  const failed = results.filter((r) => r.status === "rejected");
  if (failed.length) {
    console.error(`[email] ${failed.length}/${results.length} owner order emails failed for ${model.orderNumber}`, failed.map((f) => f.reason?.message));
  }
  return { sent: results.length - failed.length, failed: failed.length };
}

// Customer got their proof in + owners are asked to verify (manual transfer).
export async function sendPaymentSubmittedEmails(dbOrder) {
  const model = buildOrderModel(dbOrder);
  const messages = [];
  if (model.customerEmail) {
    messages.push({
      to: model.customerEmail,
      subject: `We received your payment proof — ${model.orderNumber}`,
      html: renderPaymentSubmittedCustomer(model),
      replyTo: contact.supportEmail
    });
  }
  const ownerHtml = renderPaymentSubmittedOwner(model);
  for (const owner of ownerEmails()) {
    messages.push({ to: owner, subject: `Payment to verify — ${model.orderNumber}`, html: ownerHtml, replyTo: model.customerEmail || undefined });
  }
  const results = await sendEmails(messages);
  const failed = results.filter((r) => r.status === "rejected");
  return { sent: results.length - failed.length, failed: failed.length };
}

// Customer payment verification result (manual bank transfer): approved/rejected.
export async function sendPaymentStatusEmail(dbOrder, status, notes) {
  const model = buildOrderModel(dbOrder);
  if (!model.customerEmail) return { skipped: true };
  const subject = status === "approved"
    ? `Payment approved — ${model.orderNumber}`
    : `Payment needs attention — ${model.orderNumber}`;
  return sendEmail({
    to: model.customerEmail,
    subject,
    html: renderPaymentStatusUpdate(model, status, notes),
    replyTo: contact.supportEmail
  });
}

// Customer status update (confirmed / processing / shipped / delivered / cancelled).
export async function sendOrderStatusEmail(dbOrder, status) {
  if (!shouldEmailStatus(status)) return { skipped: true };
  const model = buildOrderModel(dbOrder);
  if (!model.customerEmail) return { skipped: true };

  const label = STATUS_COPY[status]?.label || status;

  return sendEmail({
    to: model.customerEmail,
    subject: `Order ${model.orderNumber} — ${label}`,
    html: renderOrderStatusUpdate(model, status),
    replyTo: contact.supportEmail
  });
}
