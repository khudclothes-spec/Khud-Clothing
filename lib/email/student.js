...
export async function sendStudentVerificationEmail(to, token, name) {
  const content = `
    ...
    ${paragraph(`Hi ${escapeHtml(name || "there")}, enter this code on your Char Meem Clothing account to unlock student pricing:`, { muted: true })}
    ${panel(
  ...
  return sendEmail({
    to,
    subject: "Verify your student status — Char Meem Clothing",
